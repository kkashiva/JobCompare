# Project Plan
**Author**: Team 047 | CS6300 Summer2024

## 1 Introduction
The delivery of this Team project is a job comparison software application that compares job offers with salary, 
benefits, working locations, and other aspects (such as leave time per year and weekly allowed telework days). 
Users can save current job and job offers, edit current job, adjust the comparison settings, rank jobs, and compare jobs.

## 2 Process Description
### Activity: Initiate and Plan - Week of June 10th
- Description: In this phase, the team will set the project scope of work and deliverables. The team will also describe the roles and responsibilities of each team member while determining the team timeline.
- Entrance criteria: Project requirements from the instruction team and the team member assignment result.
- Exit criteria: Project plan, refined project requirement documentation, and initial draft of project documentation

### Activity: Design - Week of June 17th
- Description: In this phase, the team will draft the high-level architecture of the application.
- Entrance criteria: Initial draft of project documentation and refined project requirement documentation.
- Exit criteria: Design description document and UML diagram.

### Activity: Develop - Week of June 24th
- Description: In this phase, the team will develop the software code per design and requirements.
  - Major functions will be developed, tested, and delivered, along with the majority of UI. 
  - However, some minor functions will be polished and implemented until the end of Test phase. 
- Entrance criteria: Design description document and UML diagram.
- Exit criteria: Revised design documents, the beta version of the job comparison application, and an initial testing plan.

### Activity: Test - Week of July 1st
- Description: In this phase, the team will revisit the project requirement, and implement tests to ensure the robustness of the software.
- Entrance criteria: The beta version of the job comparison application, and an initial testing plan
- Exit criteria: Finalized testing design and the final release of the application

## 3 Team
### Team047 Members:
Thu Nguyen\
Yunhe Cui\
Kartikeya Kashiva\
Brenda Njeri
### Roles:
**Project Manager**: 
- Major responsibility: Document the progress of the project; 
ensure the software development process works as planned; track the feedback from the instruction team.
- Primary: Yunhe Cui

**Software Developer(Front/Back End)**: 
- Implement the design while keep the implementation efficient and well-written. 
- All team members should involve in the development and implementation
- Primary Development Task:
  - Thu Nguyen: Compare job offer page .xml, compare function in enter job offer page, and CompareJobOffers class. 
  - Yunhe Cui: Enter current job details, edit current job details, enter job offer detail pages (.xml), and Job class. 
  - Kartikeya Kashiva: Database and database connection for all pages
  - Brenda Njeri: Adjust comparison parameter page .xml, and EnterComparisonSettings class. 

**Quality Assurance Engineer/Tester**: 
- Major responsibility: Design testing plan; ensure the test coverage and pass rate align with the plan.
- All team members should involve in the testing plan drafting and implementation
- Primary Testing Task: align with their primary development tasks. 
